...
const add_post = async ({ command, ack, respond, client, body }) => {
  await ack();
  ...
  if (club_details) {
    ...
    if (user_is_permitted)
      await client.views.open({
        ...
        view: add_post_messg(user_id, club_name, logo_url, path).view_1,
      });
    else respond(add_post_messg(user_id).fallback_messg_not_permitted);
  } else respond(add_post_messg(user_id).fallback_messg_data_not_found);
};
